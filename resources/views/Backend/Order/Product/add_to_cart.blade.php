                ...
                <div class="row breadcrumbs-top d-inline-block">
                    <div class="breadcrumb-wrapper col-12">
                        <ol class="breadcrumb">
                            ...
                            <input type="hidden" value="{{$distributor->id}}" id="distributor">
                        </ol>
                    </div>
                </div>
            </div>
                                        ...
                                        <div class="card-content">
                                            <div class="card-body">
                                                <div class="text-right">
                                                    <a href="{{Route('admin_order.add')}}" class="btn btn-info mr-2">Continue Shopping</a>
...
<script>
   function loadCartProducts() {
    ...
    $.ajax({
        url: '{{Route('admin_order.cart_listing')}}',
        type: 'POST',
        dataType: 'json',
...
function changeQuantity(cartId, quantity) {
    var distributor=$('#distributor').val();
    $.ajax({
        url: '{{Route('admin_order.add_to_cart')}}',
        type: 'POST',
        dataType: 'json',
        data: {
            product_id: cartId,
...
function deleteCartProduct(cartId){
    $.ajax({
        url:'{{Route('admin_order.cart_delete')}}',
        type:'GET',
        dataType:'json',
        data:{
            id:cartId,
    ...
    function placeOrder()
    {
        var distributor=$('#distributor').val();
        $.ajax({
            url:'{{Route('admin_order.oderPlaced')}}',
            type:'GET',
            dataType:'json',
            data:{
                distributor:distributor,
